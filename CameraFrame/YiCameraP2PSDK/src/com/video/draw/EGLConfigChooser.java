...
public class EGLConfigChooser implements GLSurfaceView.EGLConfigChooser {
	...
	private void printConfigs(EGL10 egl, EGLDisplay display, EGLConfig[] configs) {
		...
		AntsLog.w("EGLConfigChooser", String.format("%d configurations", numConfigs));
		for (int i = 0; i < numConfigs; i++) {
			AntsLog.w("EGLConfigChooser", String.format("Configuration %d:\n", i));
	...
	private void printConfig(EGL10 egl, EGLDisplay display, EGLConfig config) {
		...
		for (int i = 0; i < attributes.length; i++) {
			...
			if (egl.eglGetConfigAttrib(display, config, attribute, value)) {
				AntsLog.w("EGLConfigChooser", String.format("  %s: %d\n", name, value[0]));
			} else {
